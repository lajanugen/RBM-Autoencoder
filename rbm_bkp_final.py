...
def test_rbm():
	...
	test_x_shared = theano.shared(np.asarray(valid_set[0], dtype=theano.config.floatX))
	...
	test = theano.function(
	    inputs=[index],
	    outputs=[sRBM.get_cost_sqr(),sRBM.fprop()],
	    givens={
	...
	#output = open('srbm2.pkl','w')
	#cPickle.dump(sRBM,output)
	#cPickle.dump(train_errors,output)
	#cPickle.dump(valid_errors,output)
	#output.close()
	
	test_error, test_code = test(0)
 	print test_error
	print Nt
	print test_code.shape
	img = Image.fromarray(tile_raster_images(X=test_code,img_shape=(28,28),tile_shape=(100,100)))
	img.save('recon.png')
